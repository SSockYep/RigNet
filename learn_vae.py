...
def main(args):
    ...
    # create model
    model = models.__dict__["skinvae"](args.nearest_bone, args.Dg, args.Lf)
    model.to(device)
...
def train(train_loader, model, optimizer, args):
    ...
    for data in train_loader:
        ...
        #kld_loss = -kl_div(mean ** 2, var, reduction='sum')
        loss += kld_loss
        loss.backward()
        optimizer.step()
...
def test(test_loader, model, args, save_result=False):
    ...
    for data in test_loader:
        ...
        with torch.no_grad():
            ...
            ce_loss = cross_entropy_with_probs(skin_pred, skin_gt, reduction='none')
            ce_loss = (ce_loss * loss_mask_batch * vert_mask.unsqueeze(1)).sum() / (loss_mask_batch * vert_mask.unsqueeze(1)).sum()
            kld_loss = - 0.5 * torch.sum(1 + var - mean.pow(2) - var.exp())
            #kld_loss = -kl_div(mean ** 2, var, reduction='sum')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_folder', default='/home/calab/RigNet/RigNetData/train/',
                        type=str, help='folder of training data')
    parser.add_argument('--val_folder', default='/home/calab/RigNet/RigNetData/val/',
                        type=str, help='folder of validation data')
    parser.add_argument('--test_folder', default='/home/calab/RigNet/RigNetData/test/',
                        type=str, help='folder of testing data')
    parser.add_argument('--nearest_bone', type=int, default=5)
    parser.add_argument('--info_folder', default='/home/calab/RigNet/RigNetData/rig_info_remesh/',
