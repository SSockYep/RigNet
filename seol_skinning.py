...
from models.SKINNING_VAE import *


def sample_latent_space(data, skinnet, sample_num):
    mean, var = skinnet.encoder(data)
    max_var_axis = int(var.argmax())
    print(torch.where(mean!=0))
    l = [mean.clone().detach() for _ in range(sample_num)]
    pred_skins = []
    for i in range(len(l)):
        l[i] += (i - sample_num // 2) * torch.exp(var) * 10
        skin = skinnet.decoder(l[i])
        pred_skins.append(skin)
    pdb.set_trace()
    return pred_skins
...
def predict_skinning(input_data, skel, skin_pred_net, surface_geodesic, mesh_filename, subsampling=False):
    """
    predict skinning
    ...
    num_nearest_bone = 5
    bones, bone_names, bone_isleaf = get_bones(skel)
    ...
    pred_rigs = sample_latent_space(data, skin_pred_net, 10)
    res = []
    
    for skin_pred in pred_rigs:
        ...
        skel_res = assemble_skel_skin(skel, skin_pred_full)
        # p [x for x in pred_rig.joint_skin if x[1] != 'Bone_dup_0' and x[1] != 'Bone_dup_1']
        #pdb.set_trace()
        p = [x for x in skel_res.joint_skin if x[1] != 'Bone_dup_0' and x[1] != 'Bone_dup_1']
        if len(p) == 0:
            pass
        res.append(skel_res)
    return res
    # return skel_res, mean, var
...
if __name__ == '__main__':
    ...
    skinVae = skinvae(nearest_bone=5, use_Dg=True, use_Lf=True)
    skinVea_checkpoint = torch.load('checkpoints/skinvae/model_best.pth.tar')
    skinVae.load_state_dict(skinVea_checkpoint['state_dict'])
    skinVae.to(device)
    skinVae.eval()
    ...
    pred_rigs = predict_skinning(data, skeleton, skinVae, surface_geodesic,
    ...
    for i in range(len(pred_rigs)):
        pred_rig = pred_rigs[i]
